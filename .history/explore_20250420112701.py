...
def fetch_media(name):
    ...
    try:
        ...
        # Remove specific unwanted sentences from the text
        unwanted_sentences = {
        ...
        if info_paragraphs:
            ...
            cleaned_sentences = [s for s in sentences if s.strip() not in unwanted_sentences or not s.strip()]
            while True:
                joined = " ".join(cleaned_sentences)
                for sentence in unwanted_sentences:
                    joined = joined.replace(sentence, "")
                if all(sentence not in joined for sentence in unwanted_sentences):
                    break
                cleaned_sentences = re.split(r"(?<=[.?!])\s+", joined)
            info_text = " ".join(cleaned_sentences).strip()
        else:
            info_text = ""
