...
def fetch_media(name):
    ...
    try:
        ...
        # Remove the first two sentences, assuming sentence ends marked by period followed by a space
        info_text_parts = re.split(r"\.\s+", info_text, maxsplit=2)
        if len(info_text_parts) > 2:
            info_text = ". ".join(info_text_parts[2:]).strip()
        else:
            info_text = ". ".join(info_text_parts[1:]).strip() if len(info_text_parts) > 1 else info_text.strip()
        ...
        info_text = ". ".join(unique_sentences)
