...
def get_bird_list():
    ...
    print("HTML snapshot:", soup.prettify()[:500])
    bird_elements = soup.select("a")
    
    for i, el in enumerate(bird_elements[:10]):
        print(f"Link {i+1}: {el.get('href')} — {el.text.strip()}")

    print(f"Found {len(bird_elements)} <a> elements.")

    birds = []
    for bird in bird_elements:
        href = bird.get("href")
        if not href or not href.startswith("https://vogelsportugal.nl/"):
            continue
        if any(x in href for x in [
            "/introductie/", "/vogelreis-", "/groepsreis", "/fotografie", "/over-ons", "/contact", "/feed"
        ]):
            continue
        name = bird.text.strip()
        if not name:
            continue
        birds.append({"name": name, "url": href})
