...
def fetch_media(name):
    ...
    try:
        ...
        # Remove leading noise (first two fragments) if they match known patterns
        info_text = re.sub(rf"^(Gezien.*?{re.escape(name)} geluid\s*)+", "", info_text)
        # Remove known pattern of location + audio prompt
        info_text = re.sub(rf"Gezien.*?{re.escape(name)} geluid", "", info_text)
