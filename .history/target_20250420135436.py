    ...
    try:
        ...
        for img in image_tags:
            ...
            if "/wp-content/uploads/" in src_lower:
                ...
                # Check if the last word and any other part of the name are in the image URL
                last_word = name_parts[-1] if name_parts else ""
                other_parts = name_parts[:-1]
                
                if last_word and last_word in src_lower:
                    if any(part in src_lower for part in other_parts if part):
                        images.append(src)
                if len(images) >= 4:
                    break
...
def main(target_name=None): # Accept optional target_name argument
    if target_name:
        ...
        bird_names = bird_names[:10] # Keep processing first 10 if no target
